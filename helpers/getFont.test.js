...
describe("getFont helper", () => {
  it("should return default font name if there's no parameters passed", () => {
    expect(getFont()).toEqual(
      `${defaultFont.name}-${weightMap[defaultFont.weight]}`
    );
  });

  it(`should return "test-bold-italic" font as parameteres are passed`, () => {
    const testFont = {
      name: "test",
      weight: "bold",
    };
    ...
    expect(getFont(testFont.name, testFont.weight, true)).toEqual(
      `${testFont.name}-${weightMap[testFont.weight]}-italic`
    );
  });
});
